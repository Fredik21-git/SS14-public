            ...
            if (now >= pylon.NextHeal)
            {
                ...
                foreach (var target in _lookup.GetEntitiesInRange(uid, pylon.HealRadius, LookupFlags.Dynamic))
                {
                    ...
                    foreach (var (dt, dmg) in damageable.Damage.DamageDict)
                    {
                        ...
                        if (bruteTypes.Contains(dt) && bruteCount > 0)
                            heal.DamageDict[dt] = -(FixedPoint2)(pylon.HealBrute / bruteCount);
                        else if (burnTypes.Contains(dt) && burnCount > 0)
                            heal.DamageDict[dt] = -(FixedPoint2)(pylon.HealBurn / burnCount);
                        else if (dt == "Bloodloss" && hasBloodloss)
